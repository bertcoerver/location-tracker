...
import {
  bandAlpha, beaconLayers, beaconTooltipHtml, fmtDistance, forecastLayers, hoverTooltipHtml,
...
test('the band fades out only at an end the half-hour cap cut', () => {
  ...
  const whole = { cutLo: false, cutHi: false };
  assert.equal(bandAlpha(0, whole), 1);
  assert.equal(bandAlpha(0.5, whole), 1);
  assert.equal(bandAlpha(1, whole), 1);

  // A cut end reaches nothing exactly at the cut, and full strength one fade in.
  const far = { cutLo: false, cutHi: true };
  ...
  const both = { cutLo: true, cutHi: true };
...
test('a crew member\'s photo is signed, because the colour alone cannot say it', () => {
  ...
  const html = mediaTooltipHtml(mediaPoi({ by: 'Mariam', crew: 'Mariam', source: 'crew' }), GUN);
  ...
  assert.doesNotMatch(mediaTooltipHtml(mediaPoi({ by: null }), GUN), /class="by"/);
});
...
test('a photographer\'s name cannot smuggle markup onto the card', () => {
  ...
  const html = mediaTooltipHtml(mediaPoi({ by: '<img src=x onerror=alert(1)>' }), GUN);
